...
class Shippo implements ShippingContract
{
    ...
    public function initialize($options = [])
    {
        ...
        if ($store->getSettingValue('marketplace_shipping_shippo_sandbox_mode')) {
            \Shippo::setApiKey($store->getSettingValue('marketplace_shipping_shippo_test_token'));
        } else {
            \Shippo::setApiKey($store->getSettingValue('marketplace_shipping_shippo_live_token'));
        }
        $this->role_id = $options['id'] ?? '';
    ...
    public function getAvailableShipping($to_address, $shippable_items, $user = null)
    {
        ...
        foreach ($shippable_items as $cart_item) {
            ...
            $parcels[] = [
                ...
                'distance_unit' => \Settings::get('marketplace_shipping_dimensions_unit', 'in'),
                'weight' => $cart_item->id->shipping['weight'] ?? $cart_item->id->product->shipping['weight'],
                'mass_unit' => \Settings::get('marketplace_shipping_weight_unit', 'oz'),
        ...
        foreach ($rates as $rate) {
            $available_rates[$this->providerName() . '|' . $rate['object_id'] . '|' . $this->rule_id] = [
