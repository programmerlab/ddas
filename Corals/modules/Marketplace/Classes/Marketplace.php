...
class Marketplace
{
    ...
    public function setTransactions($invoice, $order)
    {
        ...
        if ($applied_shipping_item && $applied_shipping_item->amount > 0) {
            $shipping_reference = explode('|', $applied_shipping_item->sku_code);

            $shipping_rule = \Corals\Modules\Marketplace\Models\Shipping::findOrFail($shipping_reference[2]);
     ...
     * @throws \Exception
     */
    public function createPaymentToken($amount, $currency, User $user)
    {
