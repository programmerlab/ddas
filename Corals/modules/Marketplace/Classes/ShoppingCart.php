...
class ShoppingCart implements ShoppingCartContract
{
    ...
    public function update()
    {
        ...
        if (user() && config('shoppingcart.cross_devices', false) && $this->authManager->check()) {
     ...
     * @param $value
     *
     * @throws \Corals\Modules\Marketplace\Exceptions\InvalidPrice
     * @throws \Corals\Modules\Marketplace\Exceptions\InvalidQuantity
     *
     * @return CartItem|null
     */
            ...
            foreach ($this->getItems() as $index => $item) {
                if ($discounted >= $totalDiscount) {
                    ...
                    if (($discounted + $itemPrice) > $totalDiscount) {
                        $totalTax += config('shoppingcart.discountTaxable', false) ? $item->tax() : $item->tax($totalDiscount - $discounted);
                    }
