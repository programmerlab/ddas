...
trait MarketplaceGallery
{
    ...
    public function gallery(Request $request, $product)
    {
        if ($request->is('*classified*')) {
            ...
            $product = \Corals\Modules\Marketplace\Models\Product::findByHash($product);
        }
        $editable = true;
    ...
    public function galleryUpload(Request $request, $product)
    {
        ...
        try {
            if ($request->has('file')) {
                ...
                $message = ['level' => 'success', 'message' => trans(trans('Marketplace::labels.product.image_upload'))];
            }
    ...
    public function galleryItemDelete(Request $request, $media)
    {
        try {
            ...
            $message = ['level' => 'success', 'message' => trans('Corals::messages.success.deleted', ['item' => trans('Marketplace::module.product.media_title')])];
        } catch (\Exception $exception) {
            $message = ['level' => 'error', 'message' => $exception->getMessage()];
    ...
    public function galleryItemFeatured(Request $request, $media)
    {
        try {
            ...
            $message = ['level' => 'success', 'message' => trans('Corals::messages.success.saved', ['item' => trans('Marketplace::module.product.media_title')])];
        } catch (\Exception $exception) {
            $message = ['level' => 'error', 'message' => $exception->getMessage()];
            log_exception($exception, Media::class, 'destroy');
        }
